...
import { checkAuth, logout, getCurrentUserWithProfile, getCurrentUser, getCurrentClinicId } from './auth.js';
import { supabase, checkSupabaseConfig } from './supabase.js';
import { showNotification, showError, showLoading, hideLoading, formatDate, formatTime, formatCurrency, closeModal } from './ui.js';
import { isToday, calculateAge, generateUUID, exportToCSV, escapeHtml } from './utils.js';
import { initializeEntraAuth, getCurrentSession } from './entra-auth.js';
import { entraConfig } from './entra-config.js';

// Expose shared utilities on window for child modules loaded via fetchAndRenderPage
...
async function loadPage(page, id = null) {
    ...
    // Store navigation context for child modules
    window.appContext = { page: page, id: id };
    ...
    switch (page) {
        ...
        case 'patient-add':
            window.appContext.mode = 'add';
            await fetchAndRenderPage('patient-add.html', './patient-form.js');
            break;

        case 'patient-view':
            await fetchAndRenderPage('patient-view.html', './patient-view.js');
            break;

        case 'patient-edit':
            window.appContext.mode = 'edit';
            await fetchAndRenderPage('patient-edit.html', './patient-form.js');
            break;
        ...
        case 'medical-history':
            await fetchAndRenderPage('medical-history.html', './medical-history.js');
            break;

        case 'appointment-add':
            window.appContext.mode = 'add';
            await fetchAndRenderPage('appointment-form.html', './appointment-form.js');
            break;

        case 'appointment-view':
            await fetchAndRenderPage('appointment-view.html', './appointment-view.js');
            break;
...
async function fetchAndRenderPage(htmlFile, modulePath) {
    ...
    try {
        ...
        if (modulePath) {
            // Cache-bust so re-visits reinitialize the controller
            const bustPath = modulePath + '?t=' + Date.now();
            const module = await import(bustPath);
