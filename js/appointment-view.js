...
class AppointmentView {
    ...
    displayAppointment() {
        ...
        document.getElementById('display-patient-name').innerHTML = 
            `<a href="#" onclick="window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-view', id: '${apt.patient.id}' } })); return false;">${apt.patient.first_name} ${apt.patient.last_name}</a>`;
    ...
    rescheduleAppointment() {
        ...
        window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'appointment-edit', id: this.appointmentId } }));
    }
    ...
    viewPatient() {
        window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-view', id: this.appointment.patient.id } }));
    }
...
export function init() {
    ...
    const appointmentId = window.appContext?.id || urlParams.get('id');
    if (!appointmentId) {
        showError('No appointment selected');
        setTimeout(() => window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'appointments' } })), 2000);
        return;
    }
    const view = new AppointmentView(appointmentId);
    view.init();
}
