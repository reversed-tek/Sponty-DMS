...
class PatientView {
    ...
    async init() {
        ...
        // Get patient ID from SPA context
        this.patientId = window.appContext?.id;
        
        if (!this.patientId) {
            showNotification('Patient ID not found', 'error');
            setTimeout(() => document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patients' } })), 2000);
            return;
        }
    ...
    setupEventListeners() {
        ...
        if (editBtn) {
            editBtn.addEventListener('click', () => {
                document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-form', id: this.patientId } }));
            });
        }
        ...
        if (clinicalNotesLink) {
            clinicalNotesLink.addEventListener('click', (e) => {
                e.preventDefault();
                document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'dental-records', id: this.patientId } }));
            });
        }
        ...
        if (medicalHistoryLink) {
            medicalHistoryLink.addEventListener('click', (e) => {
                e.preventDefault();
                document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'medical-history', id: this.patientId } }));
            });
        }
        ...
        if (dentalChartLink) {
            dentalChartLink.addEventListener('click', (e) => {
                e.preventDefault();
                document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'dental-chart', id: this.patientId } }));
            });
        }
        ...
        if (appointmentsLink) {
            appointmentsLink.addEventListener('click', (e) => {
                e.preventDefault();
                document.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'appointments', id: this.patientId } }));
            });
        }
    }
...
// SPA-compatible initializer — called by app.js dynamic import
export function init() {
    window.patientView = new PatientView();
}
