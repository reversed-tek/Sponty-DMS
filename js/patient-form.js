...
class PatientForm {
    ...
    async init() {
        ...
        // For edit mode, get patient ID from SPA context
        if (this.mode === 'edit') {
            this.patientId = window.appContext?.id;
            
            if (!this.patientId) {
                showNotification('Patient ID not found', 'error');
                setTimeout(() => window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patients' } })), 2000);
                return;
            }
    ...
    setupEventListeners() {
        ...
        if (cancelBtn) {
            cancelBtn.addEventListener('click', () => {
                if (confirm('Are you sure you want to cancel? Any unsaved changes will be lost.')) {
                    window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patients' } }));
                }
            });
        }
        ...
        if (viewBtn && this.mode === 'edit') {
            viewBtn.addEventListener('click', () => {
                window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-view', id: this.patientId } }));
            });
        }
        ...
        try {
            ...
            if (this.mode === 'add') {
                ...
                if (addAnother) {
                    ...
                    // Navigate to patient view within SPA
                    setTimeout(() => {
                        window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-view', id: result.data.id } }));
                    }, 1500);
                }
...
export function init() {
    const mode = window.appContext?.mode || 'add';
    window.patientForm = new PatientForm(mode);
}
