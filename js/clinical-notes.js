...
async function initializePage() {
    try {
        // Get patient ID from SPA context (fallback to URL for direct access)
        const patientId = window.appContext?.id || new URLSearchParams(window.location.search).get('patient_id');
...
function setupEventListeners() {
    ...
    document.getElementById('backToPatientBtn').addEventListener('click', () => {
        window.dispatchEvent(new CustomEvent('navigate', { detail: { page: 'patient-view', id: currentPatient.id } }));
    });
