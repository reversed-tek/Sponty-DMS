...
-- Remove the existing application objects only.
...
create table public.profiles (
  ...
  role public.app_role not null default 'receptionist', phone text,
  is_active boolean not null default true, last_login_at timestamptz,
  entra_object_id text unique, entra_email text, entra_display_name text,
  auth_provider text not null default 'azure', entra_synced_at timestamptz,
...
create table public.user_identities (
  ...
  provider text not null default 'azure', provider_user_id text not null,
  provider_email text, provider_metadata jsonb, is_primary boolean not null default false,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
...
create table public.auth_audit_log (
  ...
  auth_provider text, event_type text not null, entra_object_id text, success boolean,
  error_message text, metadata jsonb, created_at timestamptz not null default now()
);

create table public.patients (
  id uuid primary key default gen_random_uuid(), patient_number text not null unique,
  first_name text not null, last_name text not null,
  date_of_birth date not null check (date_of_birth <= current_date),
  gender text check (gender in ('male', 'female', 'other', 'prefer_not_to_say')),
  email text, phone text, mobile text, address text, city text, state text, postal_code text,
  emergency_contact_name text, emergency_contact_phone text,
  insurance_provider text, insurance_policy_number text, allergies text, medical_notes text,
  is_active boolean not null default true, created_by uuid references public.profiles(id),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table public.appointments (
  ...
  provider_id uuid references public.profiles(id), appointment_date date not null, appointment_time time not null,
  duration_minutes integer not null default 30 check (duration_minutes between 5 and 480),
  appointment_type text not null, status public.appointment_status not null default 'scheduled',
  reason text, notes text, cancellation_reason text, outlook_event_id text, outlook_calendar_account text,
  created_by uuid references public.profiles(id), created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table public.clinical_notes (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.patients(id) on delete cascade,
  appointment_id uuid references public.appointments(id) on delete set null, author_id uuid not null references public.profiles(id),
  visit_type text not null default 'follow_up' check (visit_type in ('initial', 'follow_up', 'emergency', 'consultation')),
  note_date date not null default current_date, subjective text, objective text, assessment text, plan text,
...
create table public.medical_history (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.patients(id) on delete cascade,
  record_type text not null check (record_type in ('condition', 'medication', 'allergy')), name text not null,
  detail text, severity text, is_active boolean not null default true, start_date date, end_date date, notes text,
  created_by uuid references public.profiles(id), created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table public.dental_chart_records (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.patients(id) on delete cascade,
  tooth_number smallint not null check (tooth_number between 11 and 48), tooth_surface text, condition text not null,
  severity text, notes text, follow_up_required boolean not null default false, recorded_by uuid not null references public.profiles(id),
  recorded_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table public.treatments (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.patients(id) on delete cascade,
  appointment_id uuid references public.appointments(id) on delete set null, provider_id uuid references public.profiles(id),
  treatment_date date not null default current_date, tooth_number smallint, procedure_code text, procedure_name text not null,
  description text, cost numeric(10,2) not null default 0 check (cost >= 0), status public.treatment_status not null default 'planned',
...
create table public.diagnoses (
  id uuid primary key default gen_random_uuid(), patient_id uuid not null references public.patients(id) on delete cascade,
  provider_id uuid references public.profiles(id), diagnosis_date date not null default current_date, diagnosis_name text not null,
  diagnosis_code text, affected_tooth text, status text not null default 'active', notes text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table public.invoices (
  ...
  appointment_id uuid references public.appointments(id) on delete set null, invoice_date date not null default current_date,
  due_date date, subtotal numeric(10,2) not null default 0 check (subtotal >= 0), tax numeric(10,2) not null default 0 check (tax >= 0),
  discount numeric(10,2) not null default 0 check (discount >= 0), total numeric(10,2) not null default 0 check (total >= 0),
  amount_paid numeric(10,2) not null default 0 check (amount_paid >= 0), balance numeric(10,2) not null default 0 check (balance >= 0),
  status public.invoice_status not null default 'pending', payment_method text, notes text, created_by uuid references public.profiles(id),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table public.invoice_items (
  ...
  treatment_id uuid references public.treatments(id) on delete set null, description text not null,
  quantity integer not null default 1 check (quantity > 0), unit_price numeric(10,2) not null default 0 check (unit_price >= 0),
  total numeric(10,2) not null default 0 check (total >= 0), created_at timestamptz not null default now()
);
...
create index profiles_entra_object_idx on public.profiles (entra_object_id);
create index user_identities_user_idx on public.user_identities (user_id);
create index user_identities_provider_idx on public.user_identities (provider, provider_user_id);
...
begin
  foreach table_name in array array['profiles','user_identities','auth_audit_log','practice_settings','patients','appointments','clinical_notes','medical_history','dental_chart_records','treatments','diagnoses','invoices','invoice_items','audit_events'] loop
...
create policy user_identities_self_or_admin on public.user_identities for select to authenticated using (user_id = auth.uid() or public.is_admin());
create policy auth_audit_log_self_or_admin on public.auth_audit_log for select to authenticated using (user_id = auth.uid() or public.is_admin());
create policy auth_audit_log_insert on public.auth_audit_log for insert to authenticated with check (user_id = auth.uid() or public.is_admin());
...
create trigger user_identities_updated_at before update on public.user_identities for each row execute function public.set_updated_at();
...
-- After creating the first user in Supabase Auth, link it as admin:
